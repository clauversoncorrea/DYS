app.controller("funcoes", function ($scope, $http, $rootScope, $compile) {
    ...
    g$.alterSargentos = function (params) {
        ...
        for (var i = 0; i < texto.length; i++) {
            if (texto[i].indexOf("|") == -1 && texto[i].indexOf("¦") == -1 && !!parseInt(texto[i])) {
                ...
                if (campo && campo.dataset.menu_id) {
        ...
        if ($(".popup[data-tela='1054']")[0]) {
            ...
            $http.post(URL + "/jsonQuery/", query).success(function (primeiro_bloco) {
                if (primeiro_bloco.data[0] && g$.memo43 != primeiro_bloco.data[0].ordem) {
                    ...
                    var query = g$.trataQuery("SELECT FICH.descricao_chamada AS e_53460, FICH.id AS e_53546, FICH.senha AS e_53458, LOCL.local AS e_53704 FROM saude.fila_chamada FICH LEFT JOIN saude.local LOCL on FICH.local_id = LOCL.id WHERE local_id in ( " + g$.memo40 + ") and COALESCE(quantidade_chamado,0)>0 order by ordem DESC limit " + g$.filaSenhaaChamar.length + ",3");
    ...
    g$.apagarcarectere = function (params, isTela) {
        var params = g$.alterSargentos(params),
            indexOf = params[1].trim(),
            valor = params[2].trim(),
            destino = $("[data-id='" + params[3].trim() + "']")[0],
            cond = params[4],
            idFuncao = params[0].split("¦")[1];
        ...
        // ultimo
        if (indexOf == "ultimo") destino.value = valor.substring(0, valor.length - 1);
        // primeiro
        else if (indexOf == "0") {
            destino.value = valor.substring(1, valor.length);
        }
        // entre 
        else {
            var valor2 = valor.substring(0, indexOf - 1);
            destino.value = valor2 + valor.substring(valor2.length + 1, valor.length);
        }
    }
